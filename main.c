...
print_stats()
{
    ...
    printf("Port 0: rx: %lu\ttx: %lu\tdrop: %lu\n", 
        port_statistics[0].rx, port_statistics[0].tx, port_statistics[0].dropped);
    port_statistics[0].rx = 0;
    port_statistics[0].tx = 0;
    port_statistics[0].dropped = 0;
}

const char dst_mac[6] = {0xb8, 0xce, 0xf6, 0x31, 0x3b, 0x56};
...
l2fwd_simple_forward(struct rte_mbuf *m, unsigned portid)
{
	...
	/* manual MAC copy instead of DPDK's unreadable one */
    if (mac_updating) {
        struct rte_ether_hdr *eth = rte_pktmbuf_mtod(m, struct rte_ether_hdr *);

        memcpy(&eth->s_addr, &l2fwd_ports_eth_addr[0], 6);
        memcpy(&eth->d_addr, dst_mac, 6);     
    }
...
main(int argc, char **argv)
{
    ...
    if (nb_ports > 1) {
        fprintf(stderr, "Warning: more than one PCI device detected, but only the first one will be used\n");
    }

	/* check port mask to possible port mask */
    // note: to use port #n, port #0 ~ #(n-1) must be visible, so this check is good at preventing misuse
	if (l2fwd_enabled_port_mask & ~((1 << nb_ports) - 1))
		rte_exit(EXIT_FAILURE, "Invalid portmask; possible (0x%x)\n",
			(1 << nb_ports) - 1);

    /* set destination for the only port */
    l2fwd_dst_ports[0] = 0;

	rx_lcore_id = 0;
	qconf = NULL;

    /* assign each available core to the only port */
    RTE_LCORE_FOREACH(lcore_id) {
        qconf = &lcore_queue_conf[lcore_id];
        /* the only rx port is port 0 */
        qconf->n_rx_port = 1;
	...
	RTE_ETH_FOREACH_DEV(portid) {
        ...
        // TODO: multuiple RX/TX queue and RSS support for a complete multi-core setting
		/* init one RX queue */
		fflush(stdout);
